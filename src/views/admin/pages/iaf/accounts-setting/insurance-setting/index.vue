<template>
  <i-layout header-key="cpsAccountPolicySelectGet" v-cloak>
    <template slot="header">
      <i-form @search="search" @reset="reset" access-key="iafSelectAccountPolicyPageList">
        ...
        <el-form-item label="生效开始日期">
          <el-date-picker
            ...
            type="date"
            :editable="false"
            @change="handleDateChange(searchParam.effectiveDate, 'params', 'effectiveDate', 'string')"
        ...
        <el-form-item label="失效结束日期">
          <el-date-picker
            ...
            type="date"
            :editable="false"
            @change="handleDateChange(searchParam.uneffectiveDate, 'params', 'uneffectiveDate', 'string')"
    ...
    <template slot="section">
        ...
        <template slot="table" slot-scope="tableScope">
          ...
          >
            ...
            <el-table-column
              ...
              :formatter="tableScope.formatter"
            ></el-table-column>
            <el-table-column label="个人缴纳" prop="empPay">
              <el-table-column
                label="基数上限"
                width="100"
                align="right"
            ...
            <el-table-column label="公司缴纳" prop="compPay">
              <el-table-column
                label="基数上限"
                align="right"
                width="100"
      ...
      >
        <detail-edit 
          ...
          :uneffectiveDate="accountInfo.uneffectiveDate"
          ></detail-edit>
      </i-dialog>
    </template>
  </i-layout>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        table: {
          ...
          columns: [
            ...
            // {prop: 'empBaseCarryRule', display: { dict: 'CARRY_RULE' }},
            {prop: 'empFixedCarryRule', display: { dict: 'CARRY_RULE' }},
            // {prop: 'compBaseCarryRule', display: { dict: 'CARRY_RULE' }}
            {prop: 'compFixedCarryRule', display: { dict: 'CARRY_RULE' }}
          ],
          setting: {
            export: {
              config: {
              ...
              key: 'iafExportAccountPolicy',
              req: config.getInsuranceList,
              param: () => ({accountId: this.accountId})
            },
            import: {
              key: 'iafImportAccountPolicy',
              business: 'iaf-account-policy',
              params: {accountCode: this.accountInfo.accountCode},
              show: () => ({ empty: false })
            },
            operator: [
              {
                key: 'iafUpdateAccountPolicy',
                text: '修改',
                func: this.modifyRow
              },
              {
                key: 'iafDeleteAccountPolicy',
                text: '删除',
                func: this.deleteRow
              }
            ],
    ...
    mounted () {
      if (!sessionStorage.getItem('INSURANCE_ACCOUNTINFO')) {
        let recordINFO = {
          ...
          uneffectiveDate: this.accountInfo.uneffectiveDate
        }
        sessionStorage.setItem('INSURANCE_ACCOUNTINFO', JSON.stringify(recordINFO))
      }
      let {accountCode, accountName} = JSON.parse(sessionStorage.getItem('INSURANCE_ACCOUNTINFO'))
      this.table.title = `户头代码：${accountCode} ` + ' ' + `户头名称：${accountName}`
    },
    beforeDestroy () {
      window.sessionStorage.removeItem('INSURANCE_ACCOUNTINFO')
    },
    methods: {
      fetch (options = {}) {
        this.load()
