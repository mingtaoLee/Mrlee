<template>
  <i-layout headerKey="cpsBankTemplatesGet">
    ...
    <template slot="section">
      ...
      >
        <BankTplModify
          v-loading="bankTplModifyLoading"
          :element-loading-text="$loadingText"
          :form="form"
...
<script>
  ...
  import BankTplModify from './bank-tpl-modify'
    ...
    methods: {
      ...
      handleSave (done) {
        return new Promise((resolve, reject) => {
          this.$refs['BankTplModify'].$refs['tax-tpl-modify'].validate((res) => {
            if (res) {
              let submitData
              if (this.form.bankTemplateId) {
                submitData = this.transformSubmitData(this.form)
                this.editBankTpl(submitData).then(() => {
                  resolve(true)
                  done()
                }).catch(() => {
                  resolve(true)
                ...
                submitData = this.transformSubmitData(this.form)
                this.createBankTpl(submitData).then(() => {
                  resolve(true)
                  done()
                }).catch(() => {
                  resolve(true)
