<template>
  <section class="salary-confirm-datail">
    <!-- 工资计算明细 -->
      ...
      headerKey="payGroupcalculateGroupCalculateIdExceptionsPost">
...
export default {
  ...
  data () {
    return {
      ...
      table: {
        ...
        setting: {
          export: {
            req: config.getSalaryFail,
            key: 'payCalculateExceptionsExport',
            config: {
              fileName: '工资计算失败明细表',
              sync: false
            },
          ...
          toolbar: [
            {
              ...
              key: 'payCalculateFailureRecomputePut',
              show: () => {
  ...
  methods: {
    ...
    // 重算
    statusRecount (Arr = {}) {
      return req('failSalaryRecompute', Arr).then(res => {
        if (res) {
