<template>
  <i-layout headerKey="cpsLowestBonusPageSelectGet">
  ...
  export default {
    ...
    data () {
      return {
        ...
        reqConfig: { req: req, getName: 'postLowests' },
        dataConfirmConfig: { req: req, name: 'lowestsConfirmData', params: {} },
        isolateConfig: { req: req, name: 'getSyncBonusData' },
        // 人事范围选择器配置
        personnelGroupProps: {
        ...
        table: {
          columns: [
            ...
            { prop: 'positionName', label: '职位', width: '120', align: 'left' },
            { prop: 'jobTitle', label: '职位分类', width: '120', align: 'left' },
            { prop: 'checkMonth', label: '核算月份', align: 'left', width: '120' },
            { prop: 'beginRunDate', label: '开业日期', align: 'left', width: '120', display: { date: '' } },
            { prop: 'salary', label: '员工工资', align: 'right', width: '120', display: { mixin: (row, col) => this.currencyFormat(row[col]) } },
            { prop: 'isFloat', label: '随时效浮动', align: 'left', width: '120', display: {dict: 'WHETHER_TYPE'} },
            { prop: 'level', label: '保底分红档级', align: 'left', width: '120', display: {dict: 'BONUS_LOWEST_LEVEL'} },
            { prop: 'levelNorm', label: '保底分红档级-标准', align: 'right', width: '160', display: { mixin: (row, col) => this.currencyFormat(row[col]) } },
            { prop: 'shouldBonus', label: '保底分红档级-应发', align: 'right', width: '160', display: { mixin: (row, col) => this.currencyFormat(row[col]) } },
            { prop: 'confirmStatus', label: '确认状态', width: '120', align: 'left', display: { dict: 'CONFIRM_STATUS' } },
            { prop: 'createBy', label: '操作者', width: '120', align: 'left' },
          ...
          setting: {
            export: {
              ...
              key: 'cpsLowestBonusExportGet',
              req: config.postLowests
            },
            import: {
              key: 'cpsLowestBonusImportGet',
              business: 'cps-lowestBonus',
              show: () => ({ empty: false })
            },
            toolbar: [{
              text: '抽数',
              key: 'cpsLowestBonusOneGet',
              func: this.isolatePolicy
            }, {
              text: '确认无误',
              key: 'cpsLowestBonusConfirm',
              func: (vm) => {
