...
//            {prop: 'orgUnitName', label: '门店名称', align: 'left', width: '140'},
            {prop: 'beginRunDate', label: '开店日期', width: '140', align: 'left', display: {date: 'default'}},
            ...
            {prop: 'lossNum', label: '亏损小店', align: 'right', width: '140'},
            {prop: 'discountNum', label: '亏损小店打折系数', align: 'right', width: '130'},
            {prop: 'storeBonus', label: '门店奖金包', align: 'right', width: '120', display: { mixin: (row, col) => this.currencyFormat(row[col]) }},
...
//            {prop: 'confirmStatus', label: '确认状态', align: 'left', width: '120', display: { dict: 'CONFIRM_STATUS' }}
          ...
          setting: {
            ...
            import: {
              key: 'cpsNewProfitBonusHalfYear',
              business: 'cps-new-store-profit-bonus',
              show: () => ({ empty: false })
            },
            toolbar: [{
              text: '抽数',
    ...
    methods: {
      ...
      init () {
        this.table.search = { checkCycleType: 2 }
      },
      approval () {
        if (this.isSelected()) {
