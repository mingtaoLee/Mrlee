<template>
  <div headerKey="cpsProfitBonusStoreCoachDetailsGet">
  ...
  export default {
    ...
    data () {
      return {
        ...
        dataConfirmConfig: { req: req, name: 'storeCoachConfirmData', params: {} },
        isolateConfig: { req: req, name: 'getNewProfitsBonusThree' },
        reqConfig: { req: req, getName: 'getStoreCoachList', deleteListName: '' },
        effectStartOptions: {
          disabledDate: (time) => {
        ...
        table: {
          columns: [
            ...
            {prop: 'storeProfit', label: '门店利润', width: '120', align: 'right', display: { mixin: (row, col) => this.currencyFormat(row[col]) }},
            {prop: 'storeBonusRate', label: '门店分红比例', width: '120', align: 'right'},
            {prop: 'storeCoef', label: '门店发放系数', width: '120', align: 'right'},
            {prop: 'storeProfitBonus', label: '门店利润分红', width: '120', align: 'right', display: { mixin: (row, col) => this.currencyFormat(row[col]) }},
          ...
          setting: {
            export: {
              ...
              req: config.getStoreCoachList,
              key: 'cpsProfitBonusStoreCoachDetailsExport'
            },
            import: {
              key: 'cpsProfitBonusStoreCoachDetailsImport',
              business: 'cps-profit-storeCoach',
              show: () => ({ empty: false })
            },
            operator: [
//              {
...
//              }
            ...
            toolbar: [{
              ...
              text: '确认无误',
              key: 'cpsProfitBonusStoreCoachDetailsPut',
              func: (vm) => {
