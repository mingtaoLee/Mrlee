  ...
  <i-layout headerKey="cpsAllowancePageSelectGet">
    ...
    <template slot="section">
      ...
      <i-dialog
        ...
        :title="(currentRow.id ? '修改' : '新增') + '福利补贴'"
        :toolbar="detailEditToolbar"
        destroyedWhenClose
      >
        <ModifyWelfare :data.sync="currentRow" ref="ModifyWelfare"></ModifyWelfare>
      </i-dialog>
    </template>
  </i-layout>
</template>
...
<script>
  ...
  import ModifyWelfare from './modify-welfare'
  ...
  export default {
    mixins: [EditorMixin, Access, PaginationMixin, SearchDateMixin, ConfirmDataMixin],
    components: {
      ModifyWelfare,
      RemoteSelect
    },
      ...
      return {
        ...
        detailEditToolbar: [
          ...
          },
          {
            ...
            text: '确认',
            func: done => {
              let tableCellVM = this.$refs['ModifyWelfare'].$refs['tableCell']
    ...
    methods: {
      ...
      handleSave (done) {
        return this.$refs['ModifyWelfare'].$refs['modify-welfare'].validate((res) => {
          let currentData = JSON.parse(JSON.stringify(this.$refs['ModifyWelfare'].form))
          currentData.checkMonth = dateFormat(currentData.checkMonth, 'YYYYMM')
          if (res) {
            if (this.currentRow.id) {
              this.modifyAllowances(currentData).then(() => {
                done()
              })
            } else {
              // this.changeEffect()
              this.addAllowances(currentData).then(() => {
                done()
              })
            }
          }
