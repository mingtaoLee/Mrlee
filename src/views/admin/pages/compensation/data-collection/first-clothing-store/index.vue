<template>
  <i-layout headerKey="cpsClotingStoreGet">
...
<script>
  ...
  import req, { config } from 'api/compensation/data-collection/first-clothing-store'
  ...
  export default {
    ...
    data () {
      return {
        ...
        reqConfig: { req: req, getName: 'postClotingStoreList' },
        dataConfirmConfig: { req: req, name: 'confirmData', params: {confirmStatus: 'Y'}, cancelParams: {confirmStatus: 'N'} },
        isolateConfig: { req: req, name: 'getClotingStoreSyncData' },
        // 薪资组选择器配置
        groupProps: {
          value: 'groupId',
          label: 'groupName'
        ...
        table: {
          columns: [
            ...
            { prop: 'sellAmount', label: '小店销售', align: 'right', width: '120' },
            { prop: 'totalHour', label: '小店总工时', align: 'right', width: '120' },
            { prop: 'prescription', label: '小店时效', align: 'right', width: '120' },
            ...
            { prop: 'level', label: '时效档级', align: 'left', width: '120', display: { dict: 'PRESCRIPTION_LEVEL' } },
            ...
            { prop: 'lastUpdateDate', label: '操作时间', align: 'left', width: '160', display: { date: 'long' } }
          ],
          setting: {
            export: {
              config: {
              ...
              key: 'cpsClothStoreExport',
              req: config.postClotingStoreList
            },
            import: {
              key: 'cpsClothingStoreImport',
              business: 'cps-clothstore',
              show: () => ({ empty: false })
            },
            toolbar: [
              {
                text: '抽数',
                key: 'cpsClotingStoreCollectGet',
                func: this.isolatePolicy
              }, {
                text: '确认无误',
                key: 'cpsClotingStoresConfirmData',
                func: (vm) => {
                ...
                text: '取消确认',
                key: 'PrescriptionClothingStorePut',
                func: (vm) => {
