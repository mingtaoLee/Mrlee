    ...
    <i-layout>
      <i-form  @reset="reset" @search="search" slot="header">
        <el-form-item label="能力项类别">
          <el-input v-model="listParams.abilityCategoryId"></el-input>
        </el-form-item>
            ...
            <el-table
              ...
              >
                ...
                <el-table-column label="学习课程" prop="courseName" width="100"  fixed="left">
                  <template slot-scope="scope">
                    <el-button @click="getCourse(scope.row.courses)" type="text">{{ scope.row.courseName }}</el-button>
                  </template>
                </el-table-column>
...
export default {
    ...
    return {
      ...
      table: {
        columns: [
          ...
          { prop: 'description', label: '任职资格/评定标准细则', width: 200, align: 'center' },
          { prop: 'scope', label: '适用人群', width: 200, align: 'center' },
          ...
          toolbar: [
            {
              text: '新增',
              type: 'primary',
              func: () => this.insertAbility()
            }, {
              text: '删除',
              type: 'danger',
              func: () => this.delAbility()
            }, {
              text: '启用',
              func: () => this.upAbility()
            }, {
              text: '禁用',
              func: () => this.disableAbility()
            }
          ],
          operator: [{
            text: '查看',
            func: this.getAbility
          }, {
            text: '修改',
            func: this.editorAbility     // 与 toolbar 类似，在 func 绑定相应的方法
          }, {
            text: '删除',
            func: this.delAbility
          }]
  ...
  methods: {
    ...
    // ---------------------------------能力项列表&查询----------------------------------------
    // 获取能力库列表
    getAbilityList (listParams) {
      req('getAbilityList', listParams)
      .then(res => {
        ...
        this.table.data.map(item => {
          item.courseName = '查看课程'
          item.scope = ''
          item.orgScope.map(sel => {
    ...
    delAbility (row) {
      ...
      if (!row && this.multipleSelection.length < 1) {
        ...
        this.multipleSelection.map(item => {
          ids += item.impAbilityId + ','
        })
        ids = ids.slice(0, -1)
      } else {
        ids = row.impAbilityId
      }
          //  获取数据，发送请求删除
      this.$confirm('确认删除?', '提示', {
        ...
        // ids.push(row.impAbilityId)
        req('delAbility', {impAbilityIds: ids})
            .then(res => {
              if (res) {
                this.$message('删除成功')
                this.getAbilityList()
