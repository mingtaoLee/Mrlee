  ...
  <div class="course">
    ...
    <i-layout>      
      <i-form  @reset="reset" @search="search" slot="header">
        <el-form-item label="课程类别">
          <el-input v-model="listParams.courseCategoryId"></el-input>
        </el-form-item>
        <el-form-item label="课程名称">
          <el-input v-model="listParams.courseName"></el-input>
        </el-form-item>
    ...
    <el-dialog :title="title" :visible.sync="userDialogVisible">
      ...
      <el-form :model="form" :rules="rules" ref='form'>
        <el-form-item label="课程类别" :label-width="formLabelWidth"  prop="courseCategoryId">
          <el-input v-model="form.courseCategoryId" style='width:400px' :disabled='lookOnly'></el-input>
        </el-form-item>
        <el-form-item label="课程编号" :label-width="formLabelWidth">
          <el-input v-model="form.courseCode" style='width:400px' :disabled='lookOnly'></el-input>
        </el-form-item>
        <el-form-item label="课程名称" :label-width="formLabelWidth"  prop="courseName">
          <el-input v-model="form.courseName" style='width:400px' :disabled='lookOnly'></el-input>
        </el-form-item>
        <el-form-item label='课程说明' :label-width="formLabelWidth" prop="intro">
          <el-input v-model="form.intro" style='width:400px' type="textarea" :disabled='lookOnly'></el-input>
        </el-form-item>
...
export default {
    ...
    return {
      ...
      table: {
        columns: [
          ...
          { prop: 'courseCategoryId', label: '课程l类型', width: 150 },
        ...
        setting: {
          ...
          operator: [{
            ...
            text: '修改',
            func: this.editorCourse     // 与 toolbar 类似，在 func 绑定相应的方法
          }, {
            text: '删除',
            func: this.delCourse
          }]
  ...
  methods: {
    ...
    // ---------------------------------人员列表----------------------------------------
    // 获取课程列表
    getCourseList (listParams) {
      req('getCourseList', listParams)
      .then(res => {
    ...
    editorCourse (row) {
      this.title = '编辑课程'
      this.addOReditor = 2
      req('findCourse', {impCourseId: row.impCourseId})
          .then(res => {
    ...
    delCourse (row) {
      ...
      this.$confirm('确认删除?', '提示', {
        ...
        req('delCourse', {impCourseIds: ids})
            .then(res => {
              if (res) {
                this.$message('删除成功')
                this.getCourseList()
