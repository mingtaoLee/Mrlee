<template lang="html">
  <div class="m-colorPicker" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    ...
    <!-- 颜色色盘 -->
    <div class="box" v-bind:class="{ open: openStatus }" v-if="openStatus">
      <div class="bd">
        <ul class="tColor">
          <li
...
export default {
  ...
  data () {
    return {
      ...
      // 标准颜色
      bColor: ['#ff1e02', '#ffc12a', '#ffff3a', '#00af57', '#00afee', '#00215f', '#72349d'],
...
.m-colorPicker{
  ...
  .box{
    position: absolute; width: 140px; background: #fff; border: 1px solid #ddd; visibility: hidden; border-radius: 2px; margin-top: 10px; box-shadow: 0 0 5px rgba(0,0,0,.15); opacity: 0; transition: all .3s ease ; z-index: 999;
  ...
  .tColor{
    li{ width: 15px; height: 15px; display: inline-block; margin: 0 2px; transition: all .3s ease;  float: left;}
