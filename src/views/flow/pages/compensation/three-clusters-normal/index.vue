<template>
  <div class="wrap">
    ...
    >
      <section class="content-follow" slot="threeClustersNormalInfo">
        <h2 class="info-title">计件计量绩效奖金（常温）</h2>
        <i-table v-if="showMap['threeClustersNormalTable']" class="info-content" :table="table" ref="iTable">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        table: {
          columns: [
            ...
            { prop: 'pieceBonus', label: '计件奖金', align: 'right' },
            { prop: 'supplierErrorOrderNum', label: '供应商差错笔数', align: 'right' },
            { prop: 'errorBonus', label: '差错奖励', align: 'center' },
    ...
    methods: {
      ...
      /** 数据填充 */
      fillIn (data) {
        this.table.data = data.threeClustersNormalments
      },
      ...
      setTableDisabled (disabledMap) {
        this.table.setting.toolbar = disabledMap.threeClustersNormalTable ? [] : this.table.setting.toolbar
        this.table.disable = disabledMap.threeClustersNormalTable
      },
      ...
      transformSubmitData (data) {
        const submitData = {}
        submitData.threeClustersNormalments = JSON.parse(JSON.stringify(data))
        ...
        // submitData.threeClustersNormalments.forEach(item => {
        //   this.datesKey.map(key => {
        //     item[key] = moment(submitData[key]).format('YYYYMM')
        //   })
        // })
        return submitData
      },
