  ...
  <div class="wrap">
    <i-process-flow
      ...
      <section class="content-follow" slot="allFlow">
        <h2 class="info-title">门店基础信息（根据申请人自动带出信息）</h2>
        ...
        <h2 class="info-title">门店属性（门店申请人填写）</h2>
        <i-table class="info-content" :table="table2" ref="table">
          <template slot="table" slot-scope="tableScope">
            <el-table :data="tableScope.clone"
                      style="width: 100%"
                      @selection-change="selectionChangeHandler(tableScope, $event)"
              ...
              <el-table-column
                v-for="(col, idx) of table2.columns || []"
                :key="idx"
                :prop="col.prop"
                :label="col.label"
                :formatter="tableScope.formatter">
  ...
  export default {
    data () {
      return {
        table: {
          columns: [
            { prop: 'credentialName', label: '门店编码' },
            { prop: 'credentialUnit', label: '大区' },
            { prop: 'credentialName', label: '区域' },
            { prop: 'credentialUnit', label: '门店名称' }
          ], // 表头
          setting: {
            // import: {
            //   business: '',
        ...
        table2: {
          columns: [
            { prop: 'credentialName', label: '开始时间' },
            { prop: 'credentialUnit', label: '生鲜小店' },
            { prop: 'credentialName', label: '食品小店' },
            { prop: 'credentialUnit', label: '服装小店' },
            { prop: 'credentialUnit', label: '加工小店' },
            { prop: 'credentialName', label: '收银小店编制' },
            { prop: 'credentialUnit', label: '后勤小店' },
            { prop: 'credentialUnit', label: '全球购小店' },
            { prop: 'credentialName', label: '电商小店' },
            { prop: 'credentialUnit', label: '鲑鱼工坊小店' },
            { prop: 'credentialUnit', label: 'Y餐厅' },
            { prop: 'beginDate', label: '麦乐巴士小店', display: { date: 'default' } },
            { prop: 'endDate', label: '备注', display: { date: 'default' } }
          ], // 表头
          setting: {
            toolbar: [{
              text: '添加',
