<template>
  <div class="wrap">
    ...
    >
      <section class="content-follow" slot="threeClustersConstantInfo">
        <h2 class="info-title">计件计量绩效奖金（定温）</h2>
        <i-table v-if="showMap['threeClustersConstantTable']" class="info-content" :table="table" ref="iTable">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        table: {
          columns: [
            ...
            { prop: 'perfSalary', label: '绩效工资', align: 'right' },
            { prop: 'targetValue', label: '目标值', align: 'right' },
            { prop: 'realValue', label: '实际值', align: 'center' },
            { prop: 'excessValue', label: '超额值', align: 'center' },
            { prop: 'piecePrice', label: '单价', align: 'center' },
            { prop: 'errorOrderNum', label: '差错笔数', align: 'right' },
            { prop: 'errorReduce', label: '差错扣款', align: 'right' },
            { prop: 'correctRate', label: '准确率', align: 'right' },
            { prop: 'bonus', label: '奖金', align: 'right' },
            { prop: 'quantifiedPerf', label: '量化绩效', align: 'right' },
            { prop: 'reduceBonus', label: '损耗奖金', align: 'right' },
            { prop: 'checkCorrectRate', label: '盘点准确率', align: 'right' },
            { prop: 'carCapacityBonus', label: '车载量奖金', align: 'right' },
            { prop: 'dispatchRate', label: '配送/配送及时率', align: 'right' },
    ...
    methods: {
      ...
      /** 数据填充 */
      fillIn (data) {
        this.table.data = data.threeClustersConstantments
      },
      ...
      setTableDisabled (disabledMap) {
        this.table.setting.toolbar = disabledMap.threeClustersConstantTable ? [] : this.table.setting.toolbar
        this.table.disable = disabledMap.threeClustersConstantTable
      },
      ...
      transformSubmitData (data) {
        const submitData = {}
        submitData.threeClustersConstantments = JSON.parse(JSON.stringify(data))
