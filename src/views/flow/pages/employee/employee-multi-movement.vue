    ...
    <!-- <i-process-flow 
      ...
      <div class="submitButton">
        <div class="button">
          ...
          <el-button type="primary" size="small" @click="submitMultiInfo" :disabled="isAble">提交</el-button>
        </div>
        <div class="button">
...
export default {
    ...
    methods: {
      ...
      submitMultiInfo () {
        ...
        let flag = true
        let message = []
        if (this.multiInfo.transFerList.length === 0) {
          this.$message('请选择需要调动的人员')
          return false
        }
        this.multiInfo.transFerList.forEach((item, i) => {
          // 必填项：调动日期，用工类型，薪资范围，薪资，工作制，调动类型，调动原因, 用工类型转换,人事范围，公司, 组织， 职位, 随时效浮动
          if (!item.transferDate || !item.newEmployeeType || !item.newPayGroupId || !item.newSalary || !item.newWorkSystem || !item.transferType || !item.newCompanyCode || !item.newPersonnelGroupCode || !item.newOrgUnitId || !item.newPosPositionId || !item.newIsAgingFloat) {
            // 已经有改人员的提示信息，则不需要在添加
          ...
          // 调动类型不为3 则必须要选择调动原因
          if (item.transferType !== '3' && !item.transferReason) {
                // 已经有改人员的提示信息，则不需要在添加
            if (message.includes(item.fullName)) {
              flag = false
              return false
          ...
          if (item.isShowSalary && (item.newSurety1 === item.newSurety2)) {
            message.push(`${item.fullName}调动异常,联保人相同`)
            flag = false
          }
