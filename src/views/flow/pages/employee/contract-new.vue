<template>
  <div class="contract-new">
   <i-process-flow 
    ...
    <!-- :createName="signForm.employeeName" -->
      <!-- <template slot="allFlow"  >
        <section class="content-follow" >  -->
    <div slot="employeeInfo" class="content-follow">
      ...
      <el-form
       ...
       v-if="signForm">
        ...
        <el-form-item label="姓名:" prop="employeeName" v-if="showMap.employeeName" class="box1">
          <el-input class="input-boxx" v-model="signForm.employeeName" :disabled="disabledMap.employeeName || !isFlowCenter"  readonly  >
          ...
          <i-personnel-select
            ...
            :multiple="false"
            filterType="newContract"
            :hasGrant="false"
            ></i-personnel-select>
        </el-form-item>
        <el-form-item label="工号:" v-if="showMap.employeeCode">
    ...
    <div slot="contactInfo" class="content-follow">
      ...
      <el-form 
       ...
       v-if="signForm">
        ...
        <el-form-item label="用工类型" prop="employeeType" v-if="showMap.employeeType">
          <i-select v-model="signForm.employeeType" parameter="EMPLOYEE_TYPE" :disabled="disabledMap.employeeType" ></i-select>
        </el-form-item>
      </el-col></el-row>

        <el-row><el-col :span="6">
        <el-form-item label="合同类型" prop="contractType" v-if="showMap.contractType">
          <!-- <i-select v-model="signForm.contractType" parameter="CONTRACT_TYPE" :disabled="disabledMap.contractType" ></i-select> -->
      ...
      <el-row>
        ...
        <el-form-item label="合同模板" prop="empTemplateId" v-if="showMap.empTemplateId">
          <div class="tel">
            <el-select
              v-model="signForm.empTemplateId" 
              clearable 
              no-data-text="该合同类型没有模板可选"
          ...
          <el-button type="text" @click="previewContract" style="margin-left: 20px" v-if="showMap.show"  >预览</el-button>
          </div>
      ...
      <el-row><el-col :span="6">
        <el-form-item label="合同主体" prop="companyName" v-if="showMap.contractSubject" required >
          <el-input v-model="signForm.companyName" disabled placeholder=""></el-input>
          <!-- <el-select v-model="signForm.contractSubject"
            ...
            <el-option
             ...
             :value="item.empCompanyCode"></el-option>
          </el-select> -->
        </el-form-item>
      </el-col></el-row>
        ...
        <el-form-item label="上班地点" prop="workAddress" v-if="showMap.workAddress">
          <el-input v-model="signForm.workAddress" :disabled="disabledMap.workAddress" ></el-input>
        </el-form-item>
        <el-form-item label="合同期限类型" prop="contractPeriodType" v-if="showMap.contractPeriodType">
          <i-select parameter="CONTRACT_PERIOD_TYPE" v-model="signForm.contractPeriodType" :disabled="disabledMap.contractPeriodType || true" placeholder=""/>
        </el-form-item>
        ...
        <el-form-item label="合同期限" prop="contractPeriod" v-if="signForm.contractPeriodType === '1' && showMap.contractPeriod">
            <el-row style="display:flex">
               <el-input v-model.number="signForm.contractPeriod" style="width:150px" :disabled="disabledMap.contractPeriod"/>
        ...
        <el-form-item label="合同结束时间" v-if="signForm.contractPeriodType == 1 && showMap.endDate">
            <el-date-picker v-model="signForm.endDate" :disabled="disabledMap.endDate || true" @change="dateChange($event,'endDate')" placeholder="" ></el-date-picker>
          <!-- <span  v-text="signForm.endDate"></span> -->
        </el-form-item>
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        dictEmpMap: ['contractType', 'contractPeriodType', 'gender', 'idType', 'domicileType', 'employeeType'],
        dictCodes: ['CONTRACT_TYPE', 'CONTRACT_PERIOD_TYPE', 'GENDER', 'ID_TYPE', 'DOMICILE_TYPE', 'EMPLOYEE_TYPE'],
        dicts: {},
        dict: {},
        employeeCluster: '' // 集群
      }
    ...
    watch: {
      ...
      employeeId (newVal, oldVal) {
        ...
        newVal && req('getcheck', {processInstanceId: this.processInstanceId, processDefineceId: this.processType, businessId: newVal})
        .then(res => {
          res && this.$message.error(`${this.signForm.employeeName}已在合同新签流程中，流程单号为${res}`)
        }).catch(err => {
      ...
      contractType (newVal, oldVal) {
        if (newVal !== oldVal) {
          ...
          //   this.signForm.contractPeriodType = '1'
          // }
        }
      },
      ...
      'signForm.contractPeriodType': {
        handler: function (newVal, oldVal) {
          console.log((newVal, this.employeeCluster, this.signForm.employeeType))
          if (newVal === '1' && this.employeeCluster === 1 && this.signForm.employeeType === '1') {
            !this.signForm.contractPeriod && (this.signForm.contractPeriod = '3')
          }
        },
        deep: true
      },
      'employeeCluster' (newVal, oldVal) {
        if (newVal === 1 && this.signForm.employeeType === '1' && this.signForm.contractPeriodType === '1') {
          !this.signForm.contractPeriod && (this.signForm.contractPeriod = '3')
        }
      },
      // 根据员工类型截取合同类型
    ...
    created () {
      ...
      this.signForm.contractType && (this.signForm.contractPeriodType = this.signForm.contractType === '12' ? '2' : '1')
    },
    methods: {
      async fillIn (data, isFlowCenter) {
        ...
        this.$watch('employeeId', this.aChangeHandler)
        // Object.assign(this.signForm, data)
      },
      // 获取合同编号
      getContractNo () {
        if (!this.signForm.empEmployeeId) {
          this.$message({message: '请先选择新签的人员', type: 'warning'})
          return false
        }
      ...
      dateChange ($event, param) {
        $event ? this.signForm[param] = moment($event).format('YYYY-MM-DD') : this.signForm[param] = ''
      },
      handleIconClick () {
      ...
      handleConfirm (selected) {
        ...
        this.signForm.orgUnitId = selected[0].unitId
      ...
      getDefaultSign (empEmployeeId) {
        ...
        new Promise(function (resolve, reject) {
          ...
          .then(data => {
            console.log(data)
            Object.assign(that.signForm, data)

            that.signForm.yearOrMonth = 'year'
            !that.signForm.isSignContract && (that.signForm.isSignContract = 'N')
            that.signForm.contractType && (that.signForm.contractPeriodType = that.signForm.contractType === '12' ? '2' : '1')
            that.signForm.startDate = moment(that.signForm.hireDate).format('YYYY-MM-DD')
            // 签订时间默认是合同开始时间
            that.signForm.signDate = moment(that.signForm.startDate).format('YYYY-MM-DD')
            delete that.signForm.signNumber
            Object.assign(that.fill, that.signForm)
            that.fill.signDate = formatDateToMi(new Date(that.signForm.signDate || new Date())).c
            that.fill.startDate = formatDateToMi(new Date(that.signForm.startDate)).c
            that.fill.endDate = formatDateToMi(new Date(that.signForm.endDate)).c
            resolve(data)
          })
        })
        .then(data => {
          // 获取集群
          let that = this
          reqd('getCluster', {orgUnitId: data.orgUnitId})
           .then(res => {
             that.employeeCluster = res
           })
        })
      },
