<template>
  <i-layout 
    ...
    :sectionPadding="0"  
    :titlePadding="3"
    header-key="insAccountingFrpayGet"
    v-loading="loading"
  ...
  >
    ...
    <div slot="section" class="mt15">
      ...
      <pay-dialog
        ...
        pageName='prepayTable'
        reasonMessage='请输入预缴原因'
        startMonthMessage='请输入预缴开始月份'
        endMonthMessage='请输入预缴结束月份'
      ></pay-dialog>
    </div>
  </i-layout>
</template>
...
export default {
    ...
    return {
      ...
      table: {
        ...
        columns: [
          ...
          // {prop: 'totalMonth', label: '预缴月数', width: '150'},
          ...
          {prop: 'reason', label: '预缴原因', width: '100', display: {dict: 'IAF_SUPPLEMENT_REASON'}},
          ...
          {label: '单位部分',
            ...
            children: [
              ...
              {prop: 'comBearAmount', label: '单位承担（个人预缴金额）', width: '100'}
            ]
          },
          {label: '个人部分',
            prop: 'PersonalPart',
            children: [
              ...
              {prop: 'empBearAmount', label: '个人承担（单位预缴金额）', width: '100'}
            ]
          },
          // {label: '滞纳金',
          //   children: [
        ...
        setting: {
          toolbar: [
            {
              text: '新增',
              key: 'insAccountingFrpayPost',
              func: this.addRecord
            },
            {
              text: '删除',
              key: 'insAccountingFrpayDelete',
              func: this.deleteRecord
            }
          ],
          import: {
            business: 'iaf-pre',
            key: 'insFrpayImportPost',
            show: () => ({empty: false})
          },
          export: {
            config: {
            ...
            key: 'insFrpayExportPost',
            req: config.Prepay
          },
          operator: [
            {
              text: '修改',
              key: 'insAccountingFrpa',
              func: this.editRow
            },
            {
              text: '删除',
  ...
  methods: {
    ...
    confirm (data) {
      ...
      this.dialogFormData.paymentType = '01'
    ...
    resetInputs () {
      ...
      this.searchData.paymentType = '01'
    },
    search () {
    ...
    editRow (row) {
      ...
      req('find', {employeeId: row.employeeId, payMonth: row.payMonth, paymentType: '01'}).then(res => {
    ...
    addRecord () {
      ...
      this.dialogTitle = '新增预缴信息'
      Object.keys(this.dialogFormData).forEach(item => {
        if (this.dialogFormData[item] !== 'prePayMonth' && this.dialogFormData[item] !== 'prePayEndMonth' && this.dialogFormData[item] !== 'paymentType' && this.dialogFormData[item] !== 'deleteComplementarityRecordId') {
