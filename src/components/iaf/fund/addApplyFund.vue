...
export default {
      ...
      table: {
        ...
        setting: {
          ...
          export: {
            ...
            req: config.fundQueryAddApplySearch,        // api 配置信息（参数名以自身业务为准）【必填】
            key: 'insInsuredHpfundIncreaseExportPost'
          },
          import: {
            business: 'iaf-hpfundIncrementApplicationImport',
            show: () => ({empty: false}),
  ...
  methods: {
    dialogConfirm (formData) {
      ...
      req('fundEditAddApply', formData).then(data => {
        if (data) {
          this.$message({
            type: 'success',
            message: '修改成功'
    ...
    editRow (data) {
      ...
      req('limitPayBase', {householdCode: data.householdCode, accountCode: data.accountCode, insuranceType: '0A', employeeCode: data.employeeCode}).then(datas => {
        let obj = {}
    ...
    deleteCruentRow (row) {
      ...
      this.$confirm('确认删除？').then(() => {
        req('hpfundIncreased', idArray).then(data => {
          if (data === true) {
            this.$message({
              type: 'success',
              message: '删除成功'
    ...
    fetch (options = {}) {
      ...
      req('fundQueryAddApplySearch', Object.assign({}, options, this.newSearchData,
    ...
    importMsg () {
      windowOpen('/flow.html#/flow/fundPayment', {processType: 'fundPayment'})
    },
    // 显示最新信息
    showNewMsg () {
      this.lastData = true
