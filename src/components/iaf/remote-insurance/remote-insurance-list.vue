<template>
  ...
  >
    <template slot="header" header-key="insExternalBehalfPo">
        <i-form @search="search(searchData)" @reset="reset" ref="form" @handleCollapse="handleCollapse">
          ...
          <el-form-item label="缴纳月份起">
            <el-date-picker v-model="searchData.payStartDate" type="month" format="yyyy-MM" :editable="false"></el-date-picker>
          </el-form-item>
          <el-form-item label="缴纳月份止">
            <el-date-picker v-model="searchData.payEndDate" type="month" format="yyyy-MM" :editable="false"></el-date-picker>
          </el-form-item>
          <el-form-item label="薪资组">
            <el-input v-model="searchData.groupName"></el-input>
          </el-form-item>
          <el-form-item label="成本中心">
            <el-input v-model="searchData.costCenterName"></el-input>
          </el-form-item>
          <el-form-item label="利润中心">
            <el-input v-model="searchData.profitCenterName"></el-input>
          </el-form-item>
...
export default {
    ...
    return {
      ...
      table: {
        columns: [
          ...
          {prop: 'employeeCode', label: '员工号', show: true, width: '100'},
          ...
          {prop: 'defaultCompanyCode', label: '应参公司代码', show: true, width: '100'},
          {prop: 'accountName', label: '实参户头', show: true, width: '150'},
          {prop: 'accountCode', label: '实参户头代码', show: true, width: '150'},
          {prop: 'companyCode', label: '实参公司代码', show: true, width: '150'},
          ...
          {prop: 'costCenterName', label: '成本中心', show: true, width: '100'},
          {prop: 'profitCenterName', label: '利润中心', show: true, width: '150'}
        ],
        setting: {
          export: {
            config: {
            ...
            text: '导出异地参保明细表',
            req: config.remoteInsurance,        // api 配置信息（参数名以自身业务为准）【必填】
            key: 'insDetailsExportPost'
          }
          // 功能暂未开始做,先把权限的key加上去
          // export: {
          //   text: '导出异地参保凭证',
          //   req: config.remoteInsurance,
          //   key: 'insExternalNonlocalStatisticsPo'
          // }
        },
        data: null,
        info: null,
  ...
  methods: {
    fetch (options = {}) {
      ...
      this.searchData.payStartDate && (this.searchData.payEndDate = new Date(Date.parse(this.searchData.payEndDate) + 3600 * 1000 * 8))
      req('remoteInsurance', Object.assign({}, options, this.searchData, this.tabsNames,
    ...
    // handleCollapse () {
    //   this.showInsuranceType = !this.showInsuranceType
    // },
    search (searchData) {
