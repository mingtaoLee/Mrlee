...
export default {
      ...
      table: {
        ...
        setting: {
          ...
          export: {
            ...
            req: config.socialAddApplyInsurance,        // api 配置信息（参数名以自身业务为准）【必填】
            key: 'insInsuredInsuranceIncreaseExportGet'
          },
          import: {
            business: 'iaf-iafAddApplicationImport',
            show: () => ({empty: false}),
  ...
  mounted () {
    this.table.search = Object.assign({}, this.searchData, {lastData: this.lastData, increasingState: true})
  },
  methods: {
    dialogConfirm (formData) {
      this.dialogStatus = false
      for (let key in formData) {
        if (key === '_id' || key === 'personnelHasAll' || key === 'unitHasAll' || key === 'payGroupHasAll' || key === 'accountHasAll' || key === 'orgUnitPath' || key === 'userId' || key === 'realmString' || key === 'applicantId' || key === 'applicantOrgId' || key === 'applicantDate') {
          delete formData[key]
        }
      }
      req('editAddApply', formData).then(data => {
        if (data) {
          this.$message({
            type: 'success',
            message: '修改成功'
    ...
    editRow (data) {
      ...
      req('limitPayBase', {householdCode: data.householdCode, accountCode: data.accountCode, insuranceType: data.insuranceType, employeeCode: data.employeeCode}).then(datas => {
        let obj = {}
        ...
        this.dialogForm = Object.assign({}, this.dialogForm, data, {empPayBase: '', compPayBase: ''})
      })
    },
    deleteCruentRow (row) {
      let idArray = []
      ...
      this.$confirm('确认删除？').then(() => {
        req('socialSecurityInsuranceIncreasedDelete', idArray).then(data => {
          if (data) {
            this.$message({
              type: 'success',
              message: '删除成功'
    ...
    fetch (options = {}) {
      ...
      req('socialAddApplyInsurance', Object.assign({}, options, this.newSearchData,
              {lastData: this.lastData, increasingState: true}
            )
         )
        .then(res => {
    ...
    search (searchData) {
      ...
      this.table.search = Object.assign({}, searchData, {lastData: this.lastData, increasingState: true})
    },
