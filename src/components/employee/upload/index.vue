  ...
  <div>
    <el-dialog
      ...
      <el-upload
        v-if="visible"
        class="upload-demo"
        action="`${$$.BASE_URL}/employee/excel/import`"
        multiple
        :limit="1"
        :auto-upload="false"
        :on-exceed="onExceed"
      ...
      <div class="sample">
        (推荐<a :href="`${$$.BASE_URL}/employee/template/download`">下载模板文件，</a>填写后上传)
...
export default {
  ...
  methods: {
    ...
    // downloadFile () {
    //   window.open('/api/org/combine/import/template')
    // },
    handleBefore (file) {
    ...
    handleSuccess (message, file, fileList) {
      this.$emit('successImport', message)
      this.closeDialog()
    },
